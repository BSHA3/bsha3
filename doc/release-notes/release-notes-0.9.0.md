...
over /Applications/Bitcoin-Qt (on Mac) or bsha3d/bsha3-qt (on Linux).
...
bsha3d makes it easier for experienced open source developers to contribute 
to the project.
...
- Add '-disablewallet' mode to let bsha3d run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with bsha3d
...
- OS X: Fix bsha3-qt startup crash when clicking dock icon
