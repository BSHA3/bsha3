...
This allows running bsha3d without having to do any manual configuration.
...
Then, to tunnel a SSL connection on 28334 to a RPC server bound on localhost on port 18334 do:

    stunnel -d 28334 -r 127.0.0.1:18334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        ProxyPassReverse http://127.0.0.1:8334/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass bsha3d rpc basic auth
...
Restarting bsha3d will no longer clear out the list of banned peers; instead
...
- #6600 `ef4945f` Include bsha3-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix bsha3-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of bsha3-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [bsha3-cli] improve error output (Jonas Schnelli)
